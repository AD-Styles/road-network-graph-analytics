...
| :---: | :--- | :--- |
| **Section 2** | **합성 도로망 생성** | 35×35 격자 기반 도로망 구성. 수평·수직·대각선 연결에 확률 기반 무작위 추가 연결 포함. <br>실제 영국 도로망 데이터(GML)의 구조를 반영하여 노드 ID에 `#` 접두사를 의도적으로 부착. |
| **Section 3** | **데이터 <br>전처리** | `str.lstrip('#')` ID 정규화 → 속도 제한 테이블 병합 → `length_s` 파생 변수 생성 → 정수 graph_id 매핑 |
| **Section 4** | **그래프 기본 분석** | 노드 1,225개 / 엣지 2,622개 / 평균 차수 4.28 확인. <br>차수 분포·도로 형태·속도 제한 시각화.<br><br><img src="plots/01_data_overview.png" width="100%"><br><br>💡 **결과 해석:** 대시보드를 통해 8종의 도로 유형별 속도 제한과 전체 연결 상태를 한눈에 파악. 대부분의 노드가 4개의 연결(차수 4)을 가지는 전형적인 격자형 도로망의 기초 통계를 보여줌. |
| **Section 5–6** | **SSSP 분석** | 최고 차수 노드(Node 383, 차수 7)를 출발지로 거리(m) · 시간(s) 각각 Dijkstra 실행. <br>SSSP 결과를 OSGB36 좌표에 색상으로 매핑하여 공간 히트맵 생성.<br><br><img src="plots/02_sssp_spatial.png" width="100%"><br><br><img src="plots/03_sssp_distribution.png" width="100%"><br><br>💡 **결과 해석:** 거리 기반 SSSP는 물리적 반경을 따라 동심원 형태로 퍼져나가는 반면, 시간 기반 SSSP는 속도 제한이 높은 도로를 따라 불규칙하게 확장됨. 가중치 기준에 따라 최단 경로 탐색 트리가 어떻게 달라지는지 시각적으로 증명. |
| **Section 7** | **중심성 5종 분석** | Degree / Betweenness / Katz / PageRank / Eigenvector 각 알고리즘 실행. <br>Top-20 노드 비교 + 지표 간 상관 행렬 도출<br><br><img src="plots/04_centrality_top20.png" width="100%"><br><br><img src="plots/05_centrality_spatial.png" width="100%"><br><br>💡 **결과 해석:** 각 알고리즘마다 '중요한 노드'를 판별하는 기준이 달라 Top-20 노드와 공간적 분포가 확연히 다르게 나타남. 특히 상관 행렬을 통해 단순 교차로(Degree)와 물류의 교두보(Betweenness)가 일치하지 않는다는 실무적 인사이트 도출. |
| **정리** | **nx-cugraph 백엔드 방식** | 3가지 활용 방식(환경변수 / 키워드 인수 / 타입 디스패치)을 코드 패턴·장단점과 함께 레퍼런스 플롯으로 정리<br><br><img src="plots/06_backend_methods.png" width="100%"><br><br><img src="plots/07_summary_card.png" width="100%"><br><br>💡 **결과 해석:** 단순 분석을 넘어, GPU 가속을 실무에 적용할 때 고려해야 할 3가지 백엔드 방식의 트레이드오프를 가이드 형태로 정리. |
